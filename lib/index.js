...
// Flags for internal callback implementations.
var consumeCb = 0,
    delegateCb = 1;
...
PullContext.prototype.write = function write(cb, err, x) {
    this.cb = cb;
    this.err = err;
    this.x = x;
};

PullContext.prototype.clear = function clear() {
    this.cb = null;
    this.err = null;
    this.x = null;
};
...
function Stream(generator) {
    ...
    self._ctx = null;
    self._is_rec_consume = false;
    self._pull_context = null;
    ...
    self._push_fn = function (err, x) {
        //console.log(self.id, '_push', self.paused);
    ...
    self._next_fn = function (xs) {
        ...
        if (xs) {
            ...
            self._generator = consumeGenerator(xs, delegatePullCb(self));
        }

        if (!self.paused) {
            if (self._in_run_generator) {
...
Stream.prototype._send = function (token) {
    ...
    if (this._request) {
        ...
        if (cb.__InternalPullCb__ != null) {
            var i = 1,
                ctx,
                s;

            if (this._pull_context) {
                ctx = this._pull_context;
                ctx.write(cb, err, x);
            }
            else {
                ctx = this._ctx;
                ...
                while (cb) {
                    ...
                    if (i === 0) {
                        ...
                        //cb(ctx.err, ctx.x);
                        if (cb.__InternalPullCb__ === consumeCb) {
                            cb.f(err, x, s._push_fn, s._next_fn);
                        }
                        else if (cb.__InternalPullCb__ === delegateCb) {
                            s._push_fn(err, x);
                            if (x !== nil) {
                                s._next_fn();
                            }
...
Stream.prototype.resume = function () {
    ...
    if (this._generator) {
        if (this._is_rec_consume) {
            this._is_rec_consume = false;
            return this;
        }
        else {
            this._runGenerator();
...
function rgInit(stream) {
    //console.log(stream.id, '_runGenerator');
    // if _generator already running, exit
    if (stream._generator_running || !stream._generator) {
        return false;
    }

    if (stream._in_run_generator) {
        stream._repeat_run_generator = true;
        return false;
    }

    stream._in_run_generator = true;
    stream._repeat_run_generator = true;

    return true;
}

function rgInternal(stream) {
    stream._repeat_run_generator = false;
    stream._generator_running = true;

    // stream._generator(stream._push_fn, stream._next_fn);

    var gen = stream._generator,
        source;
    ...
    if (gen.__ConsumeGenerator__) {
        ...
        if (source._outgoing.length) {
            ...
            source.paused = false;
            source._send(source._outgoing.dequeue());
        }
        else {
            // Inline pull.
            source._request = gen.pullCb;
            source._is_rec_consume = true;
            return source.resume();
        }
    }
    else {
        gen(stream._push_fn, stream._next_fn);
    }

    return null;
}

function rgEnd(stream) {
    stream._in_run_generator = false;
}

/*eslint-disable no-unused-vars*/
function runGenerator(stream) {
    var ctx = [],
        s = stream,
        out;
    ...
    while (s && s._repeat_run_generator) {
        out = rgInternal(s);
        if (out && rgInit(out)) {
            ctx.push(s);
            s = out;
...
Stream.prototype._runGenerator = function _runGenerator() {
    /*eslint-disable no-unused-vars*/
    var i = 2;
    /*eslint-enable no-unused-vars*/
    ...
    //     while (s && s._repeat_run_generator) {
    //         s._repeat_run_generator = false;
    //         s._generator_running = true;

    //         // s._generator(s._push_fn, s._next_fn);

    //         var gen = s._generator;
    ...
    //                 // Inline pull.
    //                 source._request = gen.pullCb;
    //                 source._is_rec_consume = true;//s._is_rec_consume + 1;
    //                 out = source.resume();

    //                 if (out) {
    //                     if (!out._generator_running && out._generator) {
    //                         out._repeat_run_generator = true;

    //                         if (!out._in_run_generator) {
    //                             out._in_run_generator = true;
    ...
    //         }
    //         else {
    //             gen(s._push_fn, s._next_fn);
    //         }

    //         while (s && !s._repeat_run_generator) {
    //             s._in_run_generator = false;
    //             if (ctx) {
    //                 s = ctx.pop();
    //             }
    //             else {
    ...
    //         }
    //         else {
    //             gen(this._push_fn, this._next_fn);
    //         }
...
function consumeGenerator(source, pullCb) {
    var gen = function () {
        self.pull(pullCb);
    };
...
function delegatePullCb(dest) {
    pullCb.__InternalPullCb__ = delegateCb;
    pullCb.s = dest;
...
Stream.prototype.consume = function (f) {
    ...
    var gen = consumeGenerator(self, pullCb);

    consumer = self._consumer = new Stream(gen);

    pullCb.__InternalPullCb__ = consumeCb;
    pullCb.s = consumer;
    pullCb.f = f;

    consumer.id = 'consume:' + consumer.id;
    consumer.source = self;

    consumer._onEnd = function _onEnd() {
        Stream.prototype._onEnd.call(this);
