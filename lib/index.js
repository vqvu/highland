...
var _ = exports = module.exports = coerce(Stream);
...
function addMethod(Stream, topLevel, name, f) {
...
var use = _.curry(function useFn(Super, originalTopLevel, methods) {
    function Sub(generator) {
        Super.call(this, generator);
    }
    inherits(Sub, Super);

    var topLevel = coerce(Sub);
    ...
    // add methods to Sub.prototype
    for (var p in methods) {
        if (hasOwn.call(methods, p)) {
            addMethod(Sub, topLevel, p, methods[p]);
        }
    }

    topLevel.use = use(Sub, topLevel);

    Sub.prototype.create = topLevel;
